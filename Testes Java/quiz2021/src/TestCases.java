...
public class TestCases {
    ...
    @Test
    public void testPizzaPriceExtra() {
        Recipe recipe = new MargueritaRecipe();

        Pizza pizza = recipe.makeMediumPizza();
        assertEquals(0, pizza.getPrice(), 0.01);
        pizza.setPrice(8.0);
        assertEquals(8.0, pizza.getPrice(), 0.01);// A sliced pizza always costs 2 more moneys
        Pizza sliced = new SlicedPizza(pizza, 8);
        assertEquals(10.0, sliced.getPrice(), 0.01);// Even if price of original pizza changes after slicing
        pizza.setPrice(6.0);
        assertEquals(8.0, sliced.getPrice(), 0.01);// If the price of the sliced pizza changes
        sliced.setPrice(4.0);
